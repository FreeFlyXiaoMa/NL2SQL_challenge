...
class WordEmbedding(nn.Module):
    def __init__(self, word_emb, N_word, gpu, SQL_TOK, our_model, trainable=False):
        ...
        if trainable:
            ...
            self.embedding.weight = nn.Parameter(
                    ...
                    #torch.from_numpy(np.ndarray(word_emb_val,dtype=np.float32)
                                     ))
        else:
    ...
    def gen_x_batch(self, q, col):  #q--Question
        B = len(q)
        val_embs = []
        ...
        if self.trainable:
            ...
            val_emb_array = np.zeros((B, max_len, self.N_word), dtype=np.float32)   #an array of 3 dimensions
            for i in range(B):
                for t in range(len(val_embs[i])):
                    val_emb_array[i,t,:] = val_embs[i][t]
            val_inp = torch.from_numpy(val_emb_array)
            if self.gpu:
                val_inp = val_inp.cuda()    #must be a tesor just can load "cuda()" function
