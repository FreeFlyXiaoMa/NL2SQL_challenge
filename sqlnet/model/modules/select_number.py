...
class SelNumPredictor(nn.Module):
    def __init__(self, N_word, N_h, N_depth, use_ca):   # N_depth--隐层数
        super(SelNumPredictor, self).__init__()
        self.N_h = N_h
        self.use_ca = use_ca

        self.sel_num_lstm = nn.LSTM(input_size=N_word, hidden_size=int(N_h/2),
    ...
    def forward(self, x_emb_var, x_len, col_inp_var, col_name_len, col_len, col_num):
        ...
        sel_num_h1 = self.sel_num_col2hid1(K_num_col).view(B, 4, int(self.N_h/2)).transpose(0,1).contiguous()
